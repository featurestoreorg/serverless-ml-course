...
START_DATE = (datetime.datetime.now() - datetime.timedelta(days=30 * 6)).strftime(
    DATE_FORMAT
)
...
def create_transactions_as_df(credit_cards: list) -> pd.DataFrame:
    ...
    transactions_df["datetime"] = transactions_df.datetime.map(
        lambda x: datetime.datetime.strptime(x, DATE_FORMAT)
    )
    transactions_df["month"] = transactions_df.datetime.map(
        lambda x: date_to_year_month(x)
    )
    ...
    fraud_labels = transactions_df[
        ["tid", "cc_num", "datetime", "month", "fraud_label"]
    ]
