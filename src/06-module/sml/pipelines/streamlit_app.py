...
credit_cards = [
    cc["cc_num"] for cc in synthetic_data.generate_list_credit_card_numbers()
]
lat = 0
long = 0
...
def get_feature_views():
    ...
    latest_record_fv = fs.get_feature_view(
        "latest_recorded_transactions_fraud_online_fv", 1
    )
...
def explore_data():
    ...
    def plot_pie(values, labels):
        fig = px.pie(
            values=values, names=labels, title="Distribution of fraud transactions"
        )
        return fig
...
def process_input_vector(cc_num, current_datetime, amount, long, lat):
    ...
    loc_delta_t_minus_1 = cc_features.haversine_distance(
        long=long,
        lat=lat,
        prev_long=latest_record_vector[3],
        prev_lat=latest_record_vector[2],
    )
    time_delta_t_minus_1 = cc_features.time_delta(
        cc_features.timestamp_to_date(latest_record_vector[0]),
...
with st.form(key="Selecting cc_num"):
    cc_num = st.selectbox("Select a credit card number.", (credit_cards))

    amount = st.slider("💶 Select withdrawal amount", 5, 1000)
...
data, current_coordinates = process_input_vector(
    cc_num=int(cc_num),
...
if st.button("📊 Make a prediction"):
    ...
    latest_recorded_transactions_fraud_online_fg = fs.get_or_create_feature_group(
        name="latest_recorded_transactions_fraud_online", version=1
    )
